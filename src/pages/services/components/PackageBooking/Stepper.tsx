  ...
  return (
    <div className="flex items-center justify-between mb-16 px-4 md:px-8 relative">
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={s.label} className="flex-1 flex items-center relative gap-3">
            {i > 0 && (
              <div
            ...
            <div className="flex items-center gap-3 relative z-10 bg-white pr-2">
              <div
                className={`flex items-center justify-center h-8 w-8 rounded-full border-2 text-[10px] font-black tracking-widest transition-all duration-300 ${
                  done
                    ? "bg-[#4988c4] border-[#4988c4] text-white shadow-xl shadow-[#4988c4]/20"
                    : active
