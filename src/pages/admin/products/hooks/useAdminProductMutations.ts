...
export function useAdminProductMutations({ state }: MutationProps) {
  ...
  const {
    editingVariant, editingProduct, setDialogOpen, setEditingProduct,
    setCreatedProductId, setCreatedProductName, setSuccessDialogOpen,
    uploadProductIdRef,
    editingCert, setEditingCert, setCertDialogOpen,
  ...
  const syncCustomizations = useCallback(async (vid: string, targetCustoms: VariantSubmitData['pendingCustoms'], localCurrent: { customizeTypeId: string; overridePrice?: number | null; overrideMultiplier?: number | null }[] = []) => {
    if (!targetCustoms) return;

    let serverItems: { customizeTypeId: string; overridePrice?: number | null; overrideMultiplier?: number | null }[] = [];
    try {
    ...
    const serverIds = new Set(serverItems.map(c => String(c.customizeTypeId || '').toLowerCase()).filter(Boolean));
    const targetIds = new Set(targetCustoms.map(c => String(c.customizeTypeId || '').toLowerCase()).filter(Boolean));
    ...
    const toUpdate = targetCustoms.filter(t => {
      ...
      const existing = serverItems.find(s => String(s.customizeTypeId || '').toLowerCase() === tid);
      if (!existing) return false;

      const oldP = existing.overridePrice == null ? null : Number(existing.overridePrice);
      const newP = t.overridePrice == null ? null : Number(t.overridePrice);

      const rawOldM = (existing as { overrideMultiplier?: number | null }).overrideMultiplier;
      const oldM = rawOldM == null ? null : Number(rawOldM);
      const newM = t.overrideMultiplier == null ? null : Number(t.overrideMultiplier);
    ...
    for (const item of toRemove) {
      await removeCustomMutation.mutateAsync({ variantId: vid, customizeTypeId: item.customizeTypeId });
    }
  ...
  const handleVariantSubmit = useCallback(async (formData: VariantSubmitData) => {
    ...
    if (vid && editingVariant) {
      try {
        ...
        if (diff > 0) {
          await addStockMutation.mutateAsync({ productVariantId: vid, quantity: diff });
        } else if (diff < 0) {
          await reduceStockMutation.mutateAsync({ productVariantId: vid, quantity: Math.abs(diff) });
        }

        const targetCustoms = formData.isCustomizable ? (formData.pendingCustoms || []) : [];
        const currentCustoms = [
          ...(editingVariant.customizeTypes || []),
          ...(editingVariant.customizeOptions || []),
          ...(editingVariant.customizeOptionGroups?.flatMap(g => g.options || []) || [])
        ];
        ...
        await Promise.all([
          ...
          queryClient.invalidateQueries({ queryKey: variantKeys.adminByProduct(state.variantProductId) }),
          queryClient.invalidateQueries({ queryKey: variantKeys.detail(vid) }),
          queryClient.invalidateQueries({ queryKey: variantKeys.customizeTypes(vid) })
        ]);
        ...
        queryClient.invalidateQueries({ queryKey: variantKeys.adminByProduct(state.variantProductId) });
        toast.error("Update Failed", "Some changes might not have been saved.");
      }
    } else {
      try {
        const variantData = {
        ...
        if (hasCustoms && formData.isCustomizable) {
          ...
          newVariantId = res.id;
          await syncCustomizations(newVariantId, formData.pendingCustoms || []);
        } else {
        ...
        if (initialStock > 0 && newVariantId) {
          await addStockMutation.mutateAsync({ productVariantId: newVariantId, quantity: initialStock });
        }

        await queryClient.invalidateQueries({ queryKey: variantKeys.adminByProduct(formData.productid) });
        state.setVariantDialogOpen(false);
        toast.success("Variant Created", "New variant and initial stock added.");
      } catch (error: unknown) {
        console.error('Variant Submission Error:', error);
        toast.error("Submission Failed", "Please check variant data and try again.");
      }
    }
  }, [
    editingVariant,
  ...
  const handleSubmit = useCallback(async (data: CreateProductRequest) => {
    const isTemplate = data.fullyCustomizedProductType && data.fullyCustomizedProductType !== 'None';
    ...
    if (!editingProduct) {
      try {
        ...
        setDialogOpen(false);
        toast.success(isTemplate ? 'Template created' : 'Product created', 'Success.');
    ...
    try {
      ...
      toast.success(isTemplate ? 'Template Updated' : 'Product Updated', 'Synced successfully.');
    } catch (error: unknown) {
      console.error('Update Error:', error);
    }
  }, [editingProduct, updateMutation, createMutation, updateTemplateMutation, createTemplateMutation, setDialogOpen, setEditingProduct, setCreatedProductId, setCreatedProductName, setSuccessDialogOpen, uploadProductIdRef, toast]);
  ...
  }, [editingCert, updateCertMutation, createCertMutation, setCertDialogOpen, setEditingCert, toast]);
  ...
  const handleConfirmDelete = useCallback((id: string) => {
    const isTemplate = state.deleteProduct?.fullyCustomizedProductType && state.deleteProduct.fullyCustomizedProductType !== 'None';
    if (isTemplate) {
      deleteTemplateMutation.mutate(id, { onSuccess: () => setDeleteProduct(null) });
    } else {
      deleteMutation.mutate(id, { onSuccess: () => setDeleteProduct(null) });
    }
  }, [deleteMutation, deleteTemplateMutation, setDeleteProduct, state.deleteProduct]);
