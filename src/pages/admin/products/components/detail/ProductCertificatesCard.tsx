                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex flex-col lg:flex-row gap-10 items-start lg:items-center">
                                <div className={cn(
                                    "w-20 h-20 rounded-[2rem] flex items-center justify-center shrink-0 group-hover:rotate-12 transition-all duration-500 border border-slate-100/20 overflow-hidden",
                                    registry.bgColor
                                )}>
                                    {registry.image ? (
                                        <img src={registry.image} alt={cert.name} className="w-full h-full object-contain p-4" />
                                    ) : (
                                        <Icon size={32} className={cn("transition-colors", registry.iconColor)} />
                                    )}
                                </div>

                                <div className="flex-1 space-y-4">
                                    <div className="space-y-1">
                                        <h4 className="text-lg font-black text-slate-900 tracking-tight leading-none group-hover:text-[#4988c4] transition-colors">
                                            {cert.name}
                                        </h4>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                            Organization: {cert.organization || registry.organization || 'Verified Institution'}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-x-8 gap-y-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                                {cert.scope || registry.scope || 'Global Standard'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6 rounded-2xl bg-slate-50/50 border border-slate-100/30 group-hover:bg-white group-hover:border-blue-100/50 transition-all duration-500 group-hover:shadow-inner-sm">
                                        <div className="flex items-start gap-4">
                                            ...
                                            <p className="text-sm font-medium text-slate-500 leading-relaxed italic line-clamp-2 uppercase tracking-tighter text-[11px]">
                                                "{cert.summary || registry.defaultDescription || 'Official recognition of adherence to rigorous quality, performance, and safety benchmarks across the entire production lifecycle.'}"
                                            </p>
                                        </div>
                                    </div>
                                </div>
