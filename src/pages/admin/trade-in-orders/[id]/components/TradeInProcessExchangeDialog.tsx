...
interface TradeInProcessExchangeDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export function TradeInProcessExchangeDialogComponent({
  ...
  const deliveryStaffs = useMemo(() => {
    const source = staffData?.items || [];
    return source.filter((staff) => {
      const role = (staff.role || "").toLowerCase();
      const position = (staff.position || "").toLowerCase();
      return role === "deliverystaff" || position === "deliverystaff";
    });
  }, [staffData]);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (!selectedStaffId) {
      toast.error("Please select a replacement delivery staff.");
      return;
    }

    if (isDamagedOutcome && !resolvedProductVariantId) {
      toast.error("This trade-in order has no valid product variant to process damaged exchange.");
      return;
    }
    ...
    try {
      ...
      const evidenceUrls = await uploadEvidenceUrls();
      ...
      toast.success(
        isDamagedOutcome
          ...
          : "Processed as ExchangeRequested.",
      );
      resetAndClose();
    } catch (error) {
      const message = error instanceof Error ? error.message : "Failed to process trade-in exchange.";
      toast.error(message);
    } finally {
      setIsUploadingEvidence(false);
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && resetAndClose()}>
      <DialogContent className="sm:max-w-[560px] p-0 overflow-hidden rounded-2xl border border-slate-200 shadow-xl gap-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-slate-100">
          <DialogTitle className="text-base font-bold text-slate-900">Process Trade-In Exchange</DialogTitle>
          <DialogDescription className="text-[13px] text-slate-500">
            Returning branch: create ExchangeRequested and replacement shipping flow.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 py-5 space-y-4 bg-white max-h-[62vh] overflow-y-auto custom-scrollbar">
          ...
          >
            {isDamagedOutcome
              ? "Damaged mode selected. System sends productVariantId and applies damaged inventory adjustment."
              : "No-damage mode selected. You can still submit exchange with note and evidence."}
          </div>

          <div className="space-y-2">
            <Label className="text-[13px] font-semibold text-slate-700">Exchange Outcome</Label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setIsDamagedSelected(false)}
              ...
              >
                <p className="text-[12px] font-semibold text-slate-800">ExchangeRequested (No Damage)</p>
                <p className="text-[11px] text-slate-500 mt-0.5">No inventory defect adjustment</p>
              </button>

              <button
                type="button"
                onClick={() => setIsDamagedSelected(true)}
              ...
              >
                <p className="text-[12px] font-semibold text-slate-800">ExchangeRequested (Damaged)</p>
                <p className="text-[11px] text-slate-500 mt-0.5">Apply damaged inventory adjustment</p>
              </button>
            </div>
          </div>

          <div className="space-y-1.5">
            <Label className="text-[13px] font-semibold text-slate-700">Replacement Delivery Staff</Label>
          ...
          <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            ...
            <label className="group flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-white px-3 py-3 text-[12px] font-semibold text-slate-600 transition hover:border-sky-300 hover:bg-sky-50/50">
              <UploadCloud className="h-4 w-4 text-primary transition group-hover:scale-105" />
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/70 flex items-center justify-end gap-2">
          ...
          <Button
            ...
            disabled={isSubmitting || !taskId || !selectedStaffId}
            className="border-0 bg-rose-600 hover:bg-rose-700 text-white shadow-none"
          >
            ...
            {isUploadingEvidence
              ...
              <span className="inline-flex items-center gap-2">
                ...
                {isDamagedOutcome ? "Record Damaged Exchange" : "Submit Exchange"}
              </span>
                )}
          </Button>
        </div>
