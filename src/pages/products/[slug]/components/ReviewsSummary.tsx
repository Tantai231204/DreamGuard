    ...
    return (
        <div className="flex flex-col items-center gap-10 rounded-[2rem] bg-slate-50/50 p-6 sm:p-8 border border-slate-100 lg:flex-row shadow-sm">
            <div className="flex flex-col items-center justify-center text-center px-4 lg:border-r lg:border-slate-100 lg:pr-10">
                <div className="text-6xl font-black text-slate-900 tracking-tighter leading-none">{averageRating.toFixed(1)}</div>
                ...
                <p className="mt-3 text-[10px] font-black uppercase text-slate-400 tracking-widest leading-none">Based on {reviews.length} Stories</p>
                <div className="mt-2 h-0.5 w-6 bg-[#4988c4] rounded-full mx-auto" />
            </div>
                ...
                {[5, 4, 3, 2, 1].map((star) => {
                    ...
                    return (
                        <div key={star} className="flex items-center gap-4 group">
                            <span className="w-10 text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-slate-900 transition-colors">{star} Star</span>
